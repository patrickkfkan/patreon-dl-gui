...
class EmbedlyDownloader {
  ...
  getPlayerURL(html) {
    ...
    if (match && match[1]) {
      ...
      try {
        ...
        if (!this.srcHostname) {
          console.log(
            `Got Embedly src "${embedlySrc}" - assume it is ${this.provider} player URL since no hostname was specified`
          );
        } else if (embedlySrcObj.hostname === this.srcHostname) {
          console.log(
            `Got ${this.provider} player URL from Embedly src: ${embedlySrc}`
          );
        } else {
          console.warn(
            `Embedly src "${embedlySrc}" does not correspond to ${this.provider} player URL`
          );
        }
        return embedlySrc;
      } catch (error) {
  ...
  async start() {
    ...
    const ytdlpPath =
      _ytdlpPath?.trim() ? path.resolve(_ytdlpPath.trim()) : null;
...
module.exports = EmbedlyDownloader;
