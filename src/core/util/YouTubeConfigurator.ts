...
export const YT_CREDS_PATH = path.join(APP_DATA_PATH, '/YouTubeCredentials.json');
...
export default class YouTubeConfigurator extends EventEmitter {
  ...
  startConnect() {
    ...
    this.#capturer.on('capture', (credentials) => {
      try {
        ...
        console.error(`Error saving credentials to "${YT_CREDS_PATH}": `, error instanceof Error ? error.message : error);
        this.emit('end', {
          status: 'error',
  ...
  resetConnectionStatus() {
    try {
      ...
      console.error(`Error deleting YouTube credentials file "${YT_CREDS_PATH}`, error instanceof Error ? error.message : error);
    }
  }
